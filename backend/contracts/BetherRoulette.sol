...
// Standard contract ownership transfer implementation,
    ...
    function placeBet(uint[] masks, uint modulo, uint commitLastBlock, uint commit, bytes32 r, bytes32 s) external payable {
        // Check that the bet is in 'clean' state.
        Bet storage bet = bets[commit];
        require (bet.gambler == address(0), "Bet should be in a 'clean' state.");
        uint betMask = masks[0];
        uint creditMask = masks[1];
...
//        require (betMask > 0 && betMask < MAX_BET_MASK, "Mask should be within range.");
      ...
      //  require (secretSigner == ecrecover(signatureHash, 27, r, s), "ECDSA signature is not valid.");
        ...
        if(modulo == MODULO_ROULETTE_MULIMODE) {
...
//        require (jackpotSize + lockedInBets <= address(this).balance, "Cannot afford to lose this bet.");
    ...
    function getDiceAmountRoulette(uint betMask, uint creditMask) private returns (uint, uint) {
        ...
        uint possibleWinAmount;
        uint jackpotFee;
        uint40[] memory masks = getNumbers(betMask);
        uint40[] memory credits = getNumbers(creditMask);
        require(masks.length == credits.length);
        for(uint i=0;i< masks.length;i++) {
            (possibleWinAmountPart, jackpotFeePart) = getDiceWinAmount(credits[i]*10000000000000000, 35, ((masks[i] * POPCNT_MULT) & POPCNT_MASK) % POPCNT_MODULO);
             possibleWinAmount+=possibleWinAmountPart;
            jackpotFee += jackpotFeePart;
        }
    }
    function getNumbers(uint input) public view returns(uint40[]) {
        uint40[] memory numbers = new uint40[](7);
        uint count = 0;
        for(uint i =0;i<7;i++) {
            uint40 number = uint40(input);
            if(number == 0) {
                break;
            }
            count++;
        ...
        return take(count, numbers);
    }

      function take(uint n, uint40[] array) internal returns(uint40[] result) {
        if (n > array.length) {
            return array;
        }
        result = new uint40[](n);
